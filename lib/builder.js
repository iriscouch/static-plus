...
Builder.prototype.prep_output = function(callback) {
  ...
  if(output_type == 'object') {
    ...
    self.prep_output_url(callback)

  else if(output_type == 'string')
    self.prep_output_dir(callback)
...
Builder.prototype.prep_output_url = function(callback) {
  var self = this
...
Builder.prototype.prep_output_dir = function(callback) {
  var self = this
...
Builder.prototype.page = function(id, content) {
  ...
  var page = content
              ? {'id':id, 'content':content} // Given two parameters
              : id                           // Given an object (presumably)
  ...
  if(output_type == 'object')
    ...
    return self.die(new Error('Not implemented: building output for ' + output_type))
}
...
Builder.prototype.output_object = function(page) {
  ...
  self.output[page.id] = page.content
  self.page_done(page)
}
...
Builder.prototype.output_dir = function(page) {
  ...
  page.path = path.join(self.output, page.id) + '.html'
...
Builder.prototype.page_done = function(page) {
  ...
  if(self.caught_up && Object.keys(self.pending).length == 0) {
    ...
    self.log.info('Deploying', {'output':self.output})
    self.deploy()
  }
}
