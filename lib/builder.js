...
Builder.prototype.doc = function(doc, source) {
  ...
  self.prep_template().on_done(function(er) {
    if(er)
      return self.die(er)

    var page = {}
    try        { page.content = self.template(doc) }
    catch (er) { return self.die(er)               }
...
Builder.prototype.output_couch = function() {
  ...
  if(self.pending.length == 0)
    return self.log.debug('No pages to output')
...
Builder.prototype.output_dir = function() {
  ...
  function write_page() {
    ...
    i += 1
    if(!page)
      return self.pages_done(pages)

    page.path = path.join(self.target, page.id) + '.html'
    self.log.debug('Write page file', {'path':page.path})
...
Builder.prototype.pages_done = function(pages) {
  ...
  if(pages.length == 0)
    return self.log.debug('Noop pages done')

  self.log.debug('Pages done', {'count':pages.length})
  pages.forEach(function(page) {
    self.pages.total += 1
    if(page.source == 'feed')
      self.pages.feed += 1
