...
Builder.prototype.prep_target_url = function(callback) {
  ...
  lib.request({uri:self.target, json:true}, function(er, res) {
    ...
    if(res.statusCode == 200 && res.body._id && res.body._rev) {
      var del = {'method':'DELETE', 'json':true, 'uri':self.target+'?rev='+res.body._rev}
      lib.request(del, function(er, res) {
        if(er)
          return callback(er)

        if(res.statusCode != 200 || !res.body.ok)
          return callback(new Error('Failed to delete target document: ' + del.uri))
  ...
  function found_doc() {
    ...
    self.log.info('Deploying to document', {'target':self.target})
