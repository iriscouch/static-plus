...
Builder.prototype.prep_template = function(callback) {
  ...
  function return_template(to_job) {
    self.emit('template', self.template)
    to_job(null, self.template)
  }

  function load_template(to_job) {
...
Builder.prototype.output_couch = function() {
  ...
  self.log.debug('Output to document', {'pages':self.pending.length})
  ...
  function attach_pages(doc, to_txn) {
    ...
    self.log.debug('Attach all pages', {'count':pages.length})
    pages.forEach(function(page) {
      doc._attachments = doc._attachments || {}

      var name     = (DEFS.namespace + '/' + page.id).replace(/\/+$/, '')
        , exists   = (name in doc._attachments)
        , data     = new Buffer(page.content)

      self.log.debug('Attach', {'name':name, 'exists':exists, 'length':data.length})

      doc._attachments[name] = {}
      doc._attachments[name].data = data.toString('base64')
  ...
  function attach_done(er, doc, result) {
    if(er) {
      self.pending = pages.concat(self.pending)
      return self.die(er)
    }
...
Builder.prototype.output_dir = function() {
  ...
  var pages = self.pending.filter(function(page) { return ! page.is_writing })
    , pages_written = []

  pages.forEach(function(page) { page.is_writing = true })
  ...
  function write_page() {
    ...
    fs.writeFile(page.path, page.content, 'utf8', function (er) {
      ...
      self.pending = self.pending.filter(function(pending_page) { return pending_page !== page })
...
Builder.prototype.pages_done = function(pages) {
  ...
  if(self.deploy_due) {
    self.log.info('Deploying', {'id':self.id, 'output':self.target})
    self.deploy()
  }
}
...
Builder.prototype.deploy = function() {
  ...
  self.log.debug('Deploying', {'id':self.id, 'target':self.target, 'pending':self.pending.length})
...
Builder.prototype.deploy_done = function(target) {
  ...
  self.log.info('Deployed', {'target':target, 'id':self.id})
