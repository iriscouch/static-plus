...
Builder.prototype.prep_source = function() {
  ...
  function check_source(callback) {
    self.log.debug('Checking source DB: ' + source_url)
    lib.request({uri:source_url, json:true}, function(er, res) {
      if(er)
        return callback(er)
      if(res.statusCode != 200)
        return callback(new Error('Bad response: ' + source_url))
      if(res.body.couchdb)
        return callback(new Error('Need a DB URL, not a Couch URL: ' + source_url))
      ...
      //  return callback(new Error('No documents in source database: ' + source_url))
...
Builder.prototype.output_couch = function(page) {
  var self = this

  self.log.debug('Output to document', {'page':page, 'pending':self.pending})
  ...
  function attach_pages(doc, to_txn) {
    ...
    var name = (DEFS.namespace + '/' + page.id).replace(/\/+$/, '')
    if(name in doc._attachments)
      self.log.warn('Page already exists', {'name':name})

    var data = new Buffer(page.content).toString('base64')
    self.log.debug('Attaching page', {'name':name, 'length':page.content.length, 'b64_length':data.length})
    doc._attachments[name] = {}
    doc._attachments[name].data         = data
    doc._attachments[name].content_type = 'text/html;charset=UTF-8'
  ...
  function attach_done(er, doc, result) {
    ...
    self.log.debug('Attached pages', {'rev':doc._rev, 'tries':result.tries, 'fetches':result.fetches})
    if(result.tries > 1)
...
Builder.prototype.deploy = function() {
  ...
  var pending_pages = Object.keys(self.pending).length
  if(pending_pages > 0) {
    self.deploy_due = true
    return self.log.debug('Deployment due when pages are done', {'pending_pages':pending_pages})
  }

  self.deploy_due = false
  self.log.debug('Deploying to URL', {'target':self.target, 'pending':self.pending})
  assert.equal(Object.keys(self.pending).length, 0, 'Must have no pending documents during deploy')
...
Builder.prototype.deploy_done = function(target) {
  ...
  self.log.info('Deployed', {'target':target})
...
Builder.prototype.deploy_url = function() {
  ...
  txn({'couch':self.target.couch, 'db':self.target.db, 'doc':self.baking_doc}, add_metadata, added_metadata)
  ...
  function copy() {
    var req = { 'method' : 'COPY'
              , 'json'   : true
              , 'uri'    : source_url
              , 'headers': { 'destination':self.target.final }
              }

    if(result.target.rev)
      req.headers.destination += '?rev=' + result.target.rev

    self.log.debug('Copying to deployment document', req)
    lib.request(req, function(er, res) {
      if(er)
        return self.die(er)
      ...
      self.log.debug('Copied', {'rev':res.body.rev})
      self.deploy_done(target_url + '?rev=' + res.body.rev)
    })
  }
}
